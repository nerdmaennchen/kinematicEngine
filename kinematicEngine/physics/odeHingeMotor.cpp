...
{
	UNUSED(timeDelta);
	const double curAngle = getCurValue();

	const double pDiff = utils::normalize(m_targetAngle - curAngle) * m_p;
	const double speedHelper = std::abs(m_maxSpeedToReachTarget);
	const double vel = utils::limited(pDiff, -speedHelper, speedHelper);
