...
protected:
	...
	/* default constructor */
	Task();
	Task(std::string name, NodeID baseNode, NodeID effectorNode, const KinematicTree &tree);
	Task(std::string name, NodeID baseNode, NodeID effectorNode, const KinematicTree &tree, TaskMethod *method);
...
public:
	...
	virtual void setMethod(TaskMethod *method) {
		m_method = method;
	}
	...
	void setActuatorsToIgnore(const std::vector<NodeID> &actuatorsToIgnore) {
		m_actuatorsToIgnore = actuatorsToIgnore;
	}

	void addActuatorToIgnore(const NodeID &actuatorToIgnore) {
	...
	const std::vector<NodeID> &getActuatorsToIgnore() const {
...
protected:
	...
	inline void removeDOFfromJacobian(arma::mat &jacobian, const KinematicTree& kinematicTree) const {
		...
		for (NodeID const &id : m_actuatorsToIgnore)
		{
			KinematicNode const* node = kinematicTree.getNode(id);
			for (kinematics::Motor2Int const& motors : node->getMotor2IntMap()) {
				jacobian.col(motors.second).zeros();
			}
		}
	}
	 ...
	 */
	std::vector<NodeID> m_actuatorsToIgnore;
